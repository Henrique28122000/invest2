...
const Dashboard: React.FC<DashboardProps> = ({ portfolio, marketAssets, cashBalance }) => {
  const [advice, setAdvice] = useState<string>('IA B3 analisando sua carteira...');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-4xl font-black mb-1 tracking-tighter uppercase">Visão Geral</h2>
          <p className="text-slate-400">Patrimônio Líquido (Ativos + Saldo)</p>
        </div>
        <div className="flex gap-4">
          <div className="px-5 py-3 bg-slate-900 border border-slate-800 rounded-2xl flex items-center gap-3">
             ...
             <div>
               <p className="text-[9px] uppercase font-black text-slate-500">Saldo em Conta</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="p-6 bg-indigo-600 rounded-3xl shadow-xl shadow-indigo-600/20">
          <p className="text-[10px] uppercase font-black text-indigo-100/60 tracking-widest mb-1">Patrimônio Líquido</p>
        ...
        <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl">
          <p className="text-[10px] uppercase font-black text-slate-500 tracking-widest mb-1">Renda Anual Proj.</p>
        ...
        <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl">
          <p className="text-[10px] uppercase font-black text-slate-500 tracking-widest mb-1">Lucro Total</p>
          ...
          <p className="text-[10px] text-slate-500 mt-1 font-bold">{summary.profitPercentage.toFixed(2)}% de retorno</p>
        </div>

        <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl">
          <p className="text-[10px] uppercase font-black text-slate-500 tracking-widest mb-1">Total Aplicado</p>
          <h3 className="text-2xl font-black text-slate-400">R$ {summary.totalInvested.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
          <p className="text-[10px] text-slate-500 mt-1 font-bold">Custo de Aquisição</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 p-8 bg-slate-900 border border-slate-800 rounded-[2.5rem] h-[450px]">
                ...
                <div key={i} className="flex items-center justify-between p-4 bg-slate-800/40 rounded-2xl border border-slate-700/50">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-indigo-600/10 rounded-xl flex flex-col items-center justify-center border border-indigo-500/20">
                       ...
                       <p className="text-lg font-black text-indigo-300">{new Date(payment.date).getDate()}</p>
                    </div>
                    <div>
                      <p className="text-sm font-black">{payment.symbol}</p>
                      <p className="text-[10px] text-slate-500 font-bold uppercase">Pagamento Previsto</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-black text-emerald-400">R$ {payment.total.toFixed(2)}</p>
                    <p className="text-[9px] text-slate-600 font-black">TOTAL A RECEBER</p>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-center text-slate-600 py-20 font-bold italic">Nenhum pagamento provisionado detectado pela IA.</p>
            )}
          </div>
        </div>

        <div className="p-8 bg-slate-900 border border-slate-800 rounded-[2.5rem]">
          <h4 className="text-lg font-bold mb-8">Composição</h4>
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={[{name: 'Ativos', value: summary.currentAssetsValue}, {name: 'Saldo', value: cashBalance}]} innerRadius={60} outerRadius={85} paddingAngle={8} dataKey="value">
                <Cell fill="#6366f1" />
                <Cell fill="#10b981" />
              </Pie>
              <Tooltip />
          ...
          <div className="mt-8 space-y-3">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-2"><div className="w-2 h-2 bg-indigo-500 rounded-full" /><span className="text-xs text-slate-400">Ações e FIIs</span></div>
                <span className="text-xs font-black">R$ {summary.currentAssetsValue.toLocaleString('pt-BR')}</span>
             </div>
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-2"><div className="w-2 h-2 bg-emerald-500 rounded-full" /><span className="text-xs text-slate-400">Saldo em Conta</span></div>
      ...
      <div className="p-8 bg-indigo-600/10 border border-indigo-500/20 rounded-[2.5rem] relative overflow-hidden group">
        ...
        <div className="relative z-10 flex items-start gap-4">
          ...
          <div>
            <h4 className="text-xl font-bold text-white mb-2">B3 Insight de Dividendos</h4>
